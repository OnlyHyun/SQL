...
## 명령문 레벨 롤백
	...
	- 단일 DML 문을 실행하는 중에 오류 발생시 해당 명령문만 롤백된다
...
## 번외) 읽기 일관성
	...
	- 읽기 일관성은 데이터에 대한 일관성 있는 뷰를 보장

	- 한 유저가 변경한 사항이 다른 유저가 변경한 사항과 충돌하지 않게 된다

	- 읽기 일관성을 동일한 데이터에 대해 다음 사항을 보장한다
		
		- 읽는 사람은 쓰는 사람의 작업이 완료되기를 기다릴 필요가 없다
